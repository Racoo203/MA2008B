...
def definir_restricciones(model, E, J, T, Q, Z, L, N, capv, jl, tc, td, tpe, a_e, capr, D, M, s):
    ...
    # Restricción 0.2: Inicializar inventario
    # Checarlo bien
    model.restr_evolucion_inventario_inicial = Constraint(E, rule=lambda m, e:
        m.inv[e, 1, T[0], Q[0]] == sum(m.xcomp[e, j, T[0]] for j in J))
    ...
    ## Restricción 0.4: Límite máximo de compra diaria (8000 unidades por día)
    #model.restr_compra_maxima = Constraint(T, rule=lambda m, t:
    #    sum(m.xcomp[e, j, t] for e in E for j in J) <= 8000)
    
    ## Restricción 0.5: Activar variable binaria de compra si hay compra ###
    #model.restr_compra_binaria = Constraint(T, rule=lambda m, t:
    #    sum(m.xcomp[e, j, t] for e in E for j in J) <= 8000 * m.compra_realizada[t])
